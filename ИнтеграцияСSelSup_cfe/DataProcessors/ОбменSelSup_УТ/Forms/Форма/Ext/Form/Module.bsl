...
Процедура СоздатьЗаказыПокупателейНаСервере() 
	...
	//ОбработкаОбъект.ВыполнитьКоманду("ОбновитьЗаказыСSelSup"); //TODO
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
...
Процедура СопоставитьНоменклатурыСпискомНаСервере()
	...
	Для каждого ТекущиеДанныеТоварыSelSup Из Объект.ТоварыSelSup Цикл
		...
		ИначеЕсли ТекущиеДанныеТовары1C.Количество() > 0 Тогда
			OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ПолучитьАртикулТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура), ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Характеристика));  //TODO
...
Процедура СписокТоваровSelSupПриАктивизацииСтроки(Элемент)
	...
	Иначе
		...
		ТекущийCBarcode = ТекущиеДанные.barcode;  
		ТекущийCArticle = ТекущиеДанные.article;
		...
		Если ЗначениеЗаполнено(ТекущийCBarcode) Тогда
			Элементы.Товары1C.ОтборСтрок = Новый ФиксированнаяСтруктура("id, barcode", ТекущийId, ТекущийCBarcode); 
		ИначеЕсли ЗначениеЗаполнено(ТекущийCArticle) Тогда
			Элементы.Товары1C.ОтборСтрок = Новый ФиксированнаяСтруктура("id, article", ТекущийId, ТекущийCArticle);	//TODO		
		Иначе
