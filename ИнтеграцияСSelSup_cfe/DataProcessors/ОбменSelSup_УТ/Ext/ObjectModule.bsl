...
Процедура ПрикрепитьЭтикеткиКДокументу(ДокументСсылка, Адрес, ИмяФайла)  
	...
	ВременныйКаталог = "C:\Users\Cleverence\Desktop\pdf\";
	...
	ПутьДляСохранения = ВременныйКаталог + "label.pdf"; 
...
Процедура ОбработатьЗаказНаКомплектацию(externalOrderId, IDЗаказа) Экспорт
	...
	Для каждого СтрокаЗаказаНаКомплектацию Из ТаблицаКодовМаркировки Цикл 
		...
		Если СтрокаЗаказаНаКомплектацию.marked 
			...
			Если Не ПрикрепитьКодМаркировкиКЗаказу(СтрокаЗаказаНаКомплектацию.ШтрихкодМаркировки, СтрокаЗаказаНаКомплектацию.orderProductCollectId) Тогда    
				ЕстьОшибки = Истина;	
			КонецЕсли;  
	...
	Иначе	
		...
		//Если УкомплектоватьЗаказ(ЗаказНаКомплектацию[0].externalOrderId, ЗаказНаКомплектацию[0].orderid) Тогда 
			ЗакрепитьЭтикетки(ДокументСсылка);
			ЗаказНаКомплектацию.Очистить();
		//КонецЕсли;  
...
Функция ПроверитьЗаказДляСборки(IDЗаказа) Экспорт 
	...
	|ГДЕ
	|	(СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ГотовКОтгрузке) ИЛИ 1=2
	|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки)
	|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ОжидаетсяОплатаПослеОтгрузки)
	|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ГотовКЗакрытию)
	|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.Закрыт))
...
Функция СобратьОтменныеПозицииИзЗаказа() Экспорт 
	...
	Если СхемаСопоставленияТовара = "Штрихкод_Штрихкод" Тогда
		...
		|ГДЕ
		|	(СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ГотовКОтгрузке) 
		|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки)
		|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ОжидаетсяОплатаПослеОтгрузки)
		|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.ГотовКЗакрытию)
		|			ИЛИ СостоянияЗаказовКлиентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовКлиентов.Закрыт))
		...
		Попытка                               
			...
			Для каждого СтрокаТовара Из МассивТоваров Цикл  
				...
				Для Каждого ТекТовар ИЗ МассивПродуктовЗаказа Цикл 
					...
					НоваяСтрока.Цена = ?(ТекТовар.Свойство("bonusPrice") и ЗначениеЗаполнено(ТекТовар.bonusPrice), ТекТовар.bonusPrice, ТекТовар.price);
					...
					Иначе 
						...
						Если countbarcodes > 0 Тогда 
							...
							Для каждого ТекущийШтрихКод Из barcodes Цикл 
								...
								ПоискНоменклатура = НайтиНоменклатуруERPПоШтрихКоду(ТекущийШтрихКод.barcode, НоваяСтрока, Истина);
						...
						ИначеЕсли ЗначениеЗаполнено(НоваяСтрока.oneCId) Тогда 						
...
Процедура СопоставитьТоварыНаСайте() Экспорт 
	...
	Для каждого ТекущиеДанныеТоварыSelSup Из ТоварыSelSup Цикл
		...
		ИначеЕсли ТекущиеДанныеТовары1C.Количество() > 0 Тогда 
			Если СхемаСопоставленияТовара = "Штрихкод_Штрихкод" Тогда
				OneCId = СвязатьТовары_SelSup_С_1С(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ПолучитьАртикулТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура), ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Характеристика));  
			Иначе
				OneCId = СвязатьТовары_SelSup_С_1С(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ПолучитьАртикулТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура), ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура)); 
			КонецЕсли;
...
Процедура УкомплектоватьЗаказыПокупателей() Экспорт 
	...
	КопияЗаказы.Свернуть("НомерЗаказа, IDЗаказа");
		...
		Для Каждого ТекТовар Из ЭтотОбъект.Заказы Цикл
			...
			Если ТекТовар.НомерЗаказа <> НомерЗаказаСтар Тогда
				...
				Иначе                                               // новый заказ 
					...
					Иначе  
						...
						Если ЭтотОбъект.ВидОперацииЗК Тогда
							ДокЗаказКлиента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;  
						Иначе
							ДокЗаказКлиента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию;  
						КонецЕсли; 
			...
			Если НЕ ЗначениеЗаполнено(ТекТовар.ЗаказКлиента) Тогда
				...
				Если ЗначениеЗаполнено(ТекТовар.Номенклатура) Тогда 
					...
					СтавкаНДС = ?(Строка(ПоискСтавкаНДС) = "Без НДС", 0, Число(Лев(ПоискСтавкаНДС, 2)));  
...
Функция ПрикрепитьКодМаркировкиКЗаказу(mark, orderProductCollectId) Экспорт 
	...
	Метод  = "/api/fbs/addMark?mark=" + mark + "&orderProductCollectId=" + orderProductCollectId;
...
Процедура ЗакрепитьЭтикетки(ДокументСсылка) Экспорт
	...
	Попытка
		...
		Пока Истина Цикл   
			...
			Если РезультатЗапроса = Неопределено 
				...
				Прервать;
			ИначеЕсли РезультатЗапроса.status = "FINISHED" Тогда  
				ПрикрепитьЭтикеткиКДокументу(ДокументСсылка, РезультатЗапроса.url, РезультатЗапроса.file); 
				Прервать;
			КонецЕсли;
...
Функция ОтправленияСоСкладаПродавца(Страница)
	...
	Метод  = "/api/fbs/findOrder?" + СтатусОтправления0 + Маркетплейс0 + "&type=FBS" + ДатаДоставкиСтрокой + "&sortBy=CREATED&limit=500&ascending=true&count=true&page=" + Страница;
...
Функция НайтиНоменклатуруERPПоШтрихКоду(ШтрихКодТовар, ВыбСтрока, ЭтоЗаказ)
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка;
	Иначе   
		Если Не ЗначениеЗаполнено(ВыбСтрока.ШтрихКод) Тогда
			Если ЭтоЗаказ = Истина Тогда
...
Функция ОбработатьСхемыСопоставленияТовара() Экспорт
	...
	СписокСхемСопоставленияТовара.Добавить("Штрихкод_Штрихкод",             "Штрихкод = Штрихкод");    
...
Функция ОбработатьТаблицуНоменклатурыSelSup(ТаблицаНоменклатурыSelSup) Экспорт 
	...
	Иначе
		...
		|	ТаблицаНоменклатурыSelSup.oneCId КАК oneCId,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.article КАК СТРОКА(20)) КАК article,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.ozonArticle КАК СТРОКА(20)) КАК ozonArticle,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.sberArticle КАК СТРОКА(20)) КАК sberArticle,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.yandexMarketShopSku КАК СТРОКА(20)) КАК yandexMarketShopSku,   
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.onecArticle КАК СТРОКА(20)) КАК onecArticle,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.aliexpress_sku КАК СТРОКА(20)) КАК aliexpress_sku,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.wb_article КАК СТРОКА(20)) КАК wb_article,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.any_article КАК СТРОКА(20)) КАК any_article,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.name КАК СТРОКА(300)) КАК name,
		|	ВЫРАЗИТЬ(ТаблицаНоменклатурыSelSup.barcode КАК СТРОКА(200)) КАК barcode
		|ПОМЕСТИТЬ ТаблицаНоменклатурыSelSup
		|ИЗ
...
Функция ЗаполнитьТаблицуНоменклатурыSelSup(TextQuery, НоменклатураSelSup) Экспорт
	...
	Пока i < НоменклатураSelSup.Количество() Цикл  
		...
		Если НоменклатураSelSup[i].Свойство("onecArticle") Тогда 
			НоваяСтрока.article = НоменклатураSelSup[i].onecArticle; 
		КонецЕсли;       
...
Функция СвязатьТовары_SelSup_С_1С(Name, Id, Article, oneCId) Экспорт 
	...
	//Для каждого ТекСтр Из МассивСвязкиТоваров Цикл
		...
		//СтруктураТовара.Вставить("externalArticle", СОКРЛП(Article));	//TODO	
