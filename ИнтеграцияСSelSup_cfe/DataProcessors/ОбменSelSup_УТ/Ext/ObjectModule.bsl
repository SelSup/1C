	...
	Попытка
		...
		Для каждого ЭлементМассива Из МассивМаркетплейсов Цикл 
			Если Не ЭлементМассива = "ONE_C"
				...
				СписокМаркетплейсов.Добавить(ЭлементМассива);
			КонецЕсли;	
		КонецЦикла;
	Исключение
...
Процедура ОбработатьКомплектации(ДанныхШтрихкода) Экспорт 
	...
	ТаблицаЗаказа.Колонки.Добавить("КодМаркировки", Новый ОписаниеТипов("Строка"));
	ТаблицаЗаказа.Колонки.Добавить("ИдентификаторCборкиТовара", Новый ОписаниеТипов("Число"));
	...
	Попытка
		...
		Для каждого СтрокаТовара Из РезультатЗапроса.products Цикл 
			...
			НоваяСтрока.КодМаркировки             = СтрокаТовара.product.view.model.category.marked; 
			НоваяСтрока.ИдентификаторCборкиТовара = СтрокаТовара.id;
	...
	Если СчетчикСтрокЗаказа = ТаблицаЗаказа.Количество() Тогда 
		...
		Если УкомплектоватьЗаказ(РезультатЗапроса.invoiceNumber, РезультатЗапроса.id) Тогда 
			//ДобавитьМаркировкуКЗаказу(ТаблицаЗаказа);
		...
		Попытка                               
			...
			Для каждого СтрокаТовара Из МассивТоваров Цикл  
				...
				Для Каждого ТекТовар ИЗ МассивПродуктовЗаказа Цикл 
					...
					ДатаДоставки = ?(СтрокаТовара.Свойство("deliveryDate") И ЗначениеЗаполнено(СтрокаТовара.deliveryDate), ПолучитьДатуОтвета(СтрокаТовара.supply.deliveryDate, "ДатаДоставки"), ТекущаяДата());
...
Процедура ДобавитьМаркировкуКЗаказу(ТаблицаЗаказа) Экспорт  
		   		
	Для Каждого СтрокаТаблицы Из ТаблицаЗаказа Цикл
		ПрикрепитьКодМаркировкиКЗаказу(СтрокаТаблицы.КодМаркировки, СтрокаТаблицы.ИдентификаторCборкиТовара);		
	КонецЦикла; 	   
...
Процедура ПрикрепитьКодМаркировкиКЗаказу(mark, orderProductCollectId) Экспорт 
	...
	Попытка
		...
		Если Не РезультатЗапроса = Неопределено Тогда
			Сообщить("Код маркировки " + orderProductCollectId + " прикреплен к заказу.");
		КонецЕсли;
	...
	Попытка
		...
		Пока Истина Цикл   
			...
			Если РезультатЗапроса.status = "FINISHED" Тогда
				ЗагрузитьPDF(РезультатЗапроса.url, РезультатЗапроса.file);
				Прервать;
...
Процедура ЗагрузитьPDF(Адрес, ИмяФайла) 
