...
Процедура ВыполнитьКоманду(ИдентификаторКоманды) Экспорт
	...
	Если ИдентификаторКоманды = "ОбновитьОстаткиНаSelSup" Тогда  
		...
		ТаблицаОрганизацийДляАвтоОбмена = ПолучитьНастройкиАвтообмена(); 
...
Процедура ОбновитьОстаткиНаСайте() Экспорт
	...
	Для каждого ТекСтр Из ТаблицаОстатков Цикл
		...
		//TODO2
		Если Не ЗначениеЗаполнено(ТекСтр.Характеристика) Тогда  
			Продолжить;
		КонецЕсли;
...
Процедура СопоставитьТоварыНаСайте() Экспорт 
	...
	Для каждого ТекущиеДанныеТоварыSelSup Из ТоварыSelSup Цикл
		...
		ИначеЕсли ТекущиеДанныеТовары1C.Количество() > 0 Тогда
			OneCId = СвязатьТовары_SelSup_С_1С(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ПолучитьАртикулТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура), ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Характеристика));  //TODO2
...
Процедура УстановитьОстатки(МассивТоваров) Экспорт
	...
	Для каждого ТекСтр Из МассивТоваров Цикл
		...
		СтруктураТовара.Вставить("serviceProductId",   Строка(ТекСтр.Характеристика.УникальныйИдентификатор())); //TODO 
...
Функция ПолучитьНастройкиАвтообмена()
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	SS_НастройкаЗаказов.Организация КАК Организация,
	|	SS_НастройкаЗаказов.Менеджер КАК Менеджер,
	|	SS_НастройкаЗаказов.Упаковка КАК Упаковка,
	|	SS_НастройкаЗаказов.СпособДоставки КАК СпособДоставки,
	|	SS_НастройкаЗаказов.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	SS_НастройкаЗаказов.НалогообложениеНДС КАК НалогообложениеНДС
	|ИЗ
	|	РегистрСведений.SS_НастройкаЗаказов КАК SS_НастройкаЗаказов
	|ГДЕ
	|	SS_НастройкаЗаказов.ВключитьВАвтообмен");  
...
Функция НайтиОрганизациюПоID(IDОрганизация, ВыбСтрока) 
	...
	Если Метаданные.РегистрыСведений.Найти("SS_НастройкаЗаказов") = Неопределено Тогда 
		Возврат Неопределено; 
	КонецЕсли;	
	
	Запрос = Новый Запрос;
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	SS_НастройкаЗаказов.ID_SelSup КАК ID_SelSup,
	|	SS_НастройкаЗаказов.Организация КАК Организация
	|ИЗ
	|	РегистрСведений.SS_НастройкаЗаказов КАК SS_НастройкаЗаказов
	|ГДЕ
	|	SS_НастройкаЗаказов.ID_SelSup = &ID";	
