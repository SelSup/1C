	...
	Иначе
		...
		Иначе
			...
			Для каждого СтрокаЗаказа Из КопияЗаказы Цикл 
				...
				Если Не ЗначениеЗаполнено(СтрокаЗаказа.warehouseId) Тогда
					Продолжить;
				КонецЕсли;	
		...
		Для Каждого ТекТовар Из ЭтотОбъект.Заказы Цикл
			...
			Если ЗначениеЗаполнено(ТекТовар.warehouseId) Тогда
				Отбор = Новый Структура();
				...
				Иначе
					ЭтотОбъект.ВыбСкладРеал = ВыбСкладПоУмолчанию;
				КонецЕсли; 
			Иначе
				ЭтотОбъект.ВыбСкладРеал = ВыбСкладПоУмолчанию;	
			КонецЕсли;
...
Функция ВыполнитьМетодSelSupAPI(Метод, ТекстЗапроса = Неопределено, ТипЗапроса = "GET", ВозвращатьСтруктуруОтвета = Истина) Экспорт
