...
&НаСервере
Процедура КомандаТест2НаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект"); 
	ОбработкаОбъект.Тест2(); 
	//ОбработкаОбъект.ВыполнитьКоманду("СформироватьURLЭтикетокЗаказовВSelSup");
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");  
КонецПроцедуры

&НаКлиенте
Процедура КомандаТест2(Команда)
	КомандаТест2НаСервере();
КонецПроцедуры
#КонецОбласти
