	...
	Если ИдентификаторКоманды = "ОбновитьЗаказыСSelSup" Тогда  
		...
		Если ТаблицаОрганизацийДляАвтоОбмена.Количество() > 0 Тогда
			...
			Для каждого СтрокаНастройки Из ТаблицаОрганизацийДляАвтоОбмена Цикл 
				...
				Если ЗначениеЗаполнено(ApiKey) Тогда  
					ЗаполнитьНайстройкиПоУмолчанию();
...
Процедура ОбработатьЗаказыНаКомплектацию(ДанныеШтрихкода) Экспорт
	...
	Попытка
		...
		Для каждого СтрокаЗаказа Из РезультатЗапроса Цикл 
			...
			Если СтрокаЗаказа.product.Свойство("oneCId") И ЗначениеЗаполнено(СтрокаЗаказа.product.oneCId) Тогда
				НоваяСтрока.oneCId        = Новый УникальныйИдентификатор(СокрЛП(СтрокаЗаказа.product.oneCId)); //TODO   
			...
			Для каждого СтрокаТовара Из МассивТоваров Цикл  
				...
				Для Каждого ТекТовар ИЗ МассивПродуктовЗаказа Цикл 
					...
					Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
						...
						//Если ЗначениеЗаполнено(НоваяСтрока.ШтрихКод) Тогда
						//	ПоискХарактеристика =  НайтиХарактеристикуТовара(НоваяСтрока.Номенклатура, НоваяСтрока.ШтрихКод);
						//	Если ЗначениеЗаполнено(ПоискХарактеристика) Тогда    
						//		НоваяСтрока.Характеристика = ПоискХарактеристика.Характеристика;  
						//	Иначе	
...
Процедура СформироватьРеализации(Док) Экспорт
	...
	//Если Не СозданиеРеализаций Тогда
	//	Возврат;
	//КонецЕсли;	
...
Процедура ПрикрепитьКодМаркировкиКЗаказу(mark, orderProductCollectId) Экспорт 
...
Процедура ЗаполнитьНайстройкиПоУмолчанию() 	
...
Функция ПолучитьНастройкиАвтообмена(ИдентификаторКоманды) 
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	SS_НастройкаОбмена.Организация КАК Организация,
	|	SS_НастройкаОбмена.Менеджер КАК Менеджер,
	|	SS_НастройкаОбмена.Упаковка КАК Упаковка,
	...
	//|	SS_НастройкаОбмена.Склад КАК Склад,
	|	SS_НастройкаОбмена.ПричинаОтменыЗаказа КАК ПричинаОтменыЗаказа,
	|	SS_НастройкаОбмена.ПричинаОтменыЗаказаПокупателем КАК ПричинаОтменыЗаказаПокупателем,
	...
	|ГДЕ
	|	SS_НастройкаОбмена.ВключитьВАвтообменОстатками"); 
...
Функция ЗаполнитьТаблицуНоменклатурыSelSup(TextQuery, НоменклатураSelSup) Экспорт
	...
	Пока i < НоменклатураSelSup.Количество() Цикл  
		...
		КонецЕсли;
		Если НоменклатураSelSup[i].view.model.Свойство("yandexMarketShopSku") Тогда 
