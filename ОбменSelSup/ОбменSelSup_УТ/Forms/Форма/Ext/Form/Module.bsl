...
Процедура СопоставитьНоменклатурыСпискомНаСервере()
	...
	Для каждого ТекущиеДанныеТоварыSelSup Из Объект.ТоварыSelSup Цикл
		...
		ИначеЕсли ТекущиеДанныеТовары1C.Количество() > 0 Тогда 
			Если Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод" Тогда
				OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ПолучитьАртикулТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура), ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Характеристика));  			
			Иначе
...
Процедура ПродолжитьОперациюСопоставленияТоваров(РезультатВопроса, ПараметрыЗаписи) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если Не ТекущиеДанныеТоварыSelSup = Неопределено 
			...
			Если Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод" Тогда
				OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ТекущиеДанныеТовары1C.article, ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C.Характеристика));   //TODO
			Иначе
