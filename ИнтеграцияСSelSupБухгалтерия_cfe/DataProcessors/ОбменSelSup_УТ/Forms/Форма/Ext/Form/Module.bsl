...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//1//Объект.ИмяМетаданных = Метаданные.Имя;
	//1//ПравоРегистрацииШтрихкодовНоменклатурыДоступно = ШтрихкодированиеНоменклатуры.ПравоРегистрацииШтрихкодовНоменклатурыДоступно();
	//1//ПравоРегистрацииШтрихкодовНоменклатурыДоступно = ШтрихкодированиеНоменклатурыБПКлиентСервер.ПараметрыОбработкиШтрихкодов();
...
Процедура ПриОткрытии(Отказ)
	...
	Объект.ЧислоТоваров = 2000;
	
	Если Не ЗначениеЗаполнено(Объект.СхемаСопоставленияТовара) Тогда
		Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод";
	КонецЕсли;	
...
Процедура ИзменитьServicesНаСервере(Service = Неопределено) 
	...
	Если Не Service = Неопределено Тогда  
		...
		УдалитьService = ОпределитьАктивностьServiceНаСервере("ИзменитьServices_", Service); 
...
&НаКлиенте
Процедура ИзменитьServicesImport_WILDBERRIES()    
	ИзменитьServicesImportНаСервере("WILDBERRIES"); 
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_OZON() 
	ИзменитьServicesImportНаСервере("OZON");  
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_YANDEX_MARKET()     
	ИзменитьServicesImportНаСервере("YANDEX_MARKET");  
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_SBER_MEGA_MARKET() 
	ИзменитьServicesImportНаСервере("SBER_MEGA_MARKET"); 
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_AVITO()  
	ИзменитьServicesImportНаСервере("AVITO"); 
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_LEROY_MERLIN() 
	ИзменитьServicesImportНаСервере("LEROY_MERLIN");  
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_DETMIR() 
	ИзменитьServicesImportНаСервере("DETMIR"); 
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_KAZAN_EXPRESS() 
	ИзменитьServicesImportНаСервере("KAZAN_EXPRESS"); 
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаКлиенте
Процедура ИзменитьServicesImport_ALIE_EXPRESS() 
	ИзменитьServicesImportНаСервере("ALIE_EXPRESS");  
	УстановитьОформлениеИмпортаТовараНаСервере();
КонецПроцедуры 

&НаСервере
Процедура ИзменитьServicesImportНаСервере(Service = Неопределено) 
	...
	Если Не Service = Неопределено Тогда 
		...
		УдалитьService = Ложь; 		
		Если Элементы["ИзменитьServicesImport_" + Service].ЦветФона = ЦветаСтиля.ЦветАктивности Тогда 
			УдалитьService = Истина;  
			Service = Неопределено;
		КонецЕсли;
		
		ОбработкаОбъект.ServicesImport = ?(УдалитьService, "", "&" + nameparam + "[]=" + Service);      
		
		//фон кнопки
		СписокМаркетплейсов = СписокМаркетплейсовНаСервере(); 
		Для каждого ЭлементСписка Из СписокМаркетплейсов Цикл	
			ОпределитьАктивностьКнопкиНаСервере("ИзменитьServicesImport_", ЭлементСписка.Значение, ЭлементСписка.Представление, Service, ЦветаСтиля.ЦветАктивности); 
		КонецЦикла;  
...
&НаСервере
Процедура ОпределитьАктивностьКнопкиНаСервере(ГруппаЭлементов, КлючЭлемента, ЗначениеЭлемента, ТекущееЗначение, ЦветАктивности) 
	
	Если Элементы.Найти(ГруппаЭлементов + КлючЭлемента) = Неопределено Тогда 
		Возврат;
	КонецЕсли;	
	
	Если ЗначениеЭлемента = ТекущееЗначение Тогда 
		Элементы[ГруппаЭлементов + КлючЭлемента].ЦветФона = ЦветАктивности;
	Иначе                                                               
		Элементы[ГруппаЭлементов + КлючЭлемента].ЦветФона = ЦветаСтиля.ЦветФонаКнопки; 
	КонецЕсли; 
	
КонецПроцедуры

&НаСервере
Функция ОпределитьАктивностьServiceНаСервере(ГруппаЭлементов, Service) 
	
	УдалитьService = Ложь; 
	
	Если Элементы[ГруппаЭлементов + Service].ЦветФона = ЦветаСтиля.ЦветАктивности Тогда 
		Элементы[ГруппаЭлементов + Service].ЦветФона = ЦветаСтиля.ЦветФонаКнопки;
		УдалитьService = Истина;
	Иначе                                                               
		Элементы[ГруппаЭлементов + Service].ЦветФона = ЦветаСтиля.ЦветАктивности; 
	КонецЕсли; 
...
Процедура ИзменитьМаркетплейсНаКлиенте(Маркетплейс) 
	...
	Если Объект.Склады.Количество() = 0 Тогда
	    ПоказатьПредупреждение(,"Заполните склады на закладке настроек справочников!");	
	Иначе	   		                  
...
Процедура ИзменитьМаркетплейсНаСервере(Маркетплейс = "")
	...
	СписокМаркетплейсов = СписокМаркетплейсовНаСервере();  
	Для каждого ЭлементСписка Из СписокМаркетплейсов Цикл	
		ОпределитьАктивностьКнопкиНаСервере("ИзменитьМаркетплейс_", ЭлементСписка.Значение, ЭлементСписка.Представление, Маркетплейс, ЦветаСтиля.ЦветАктивности); 
	КонецЦикла;
...
Процедура ИзменитьСтатусОтправленияНаСервере(СтатусОтправления = "")
	...
	СписокСтатусовОтправлений = СписокСтатусовОтправленийНаСервере();  
	Для каждого ЭлементСписка Из СписокСтатусовОтправлений Цикл	
		ОпределитьАктивностьКнопкиНаСервере("ИзменитьСтатусОтправления", ЭлементСписка.Значение, ЭлементСписка.Представление, СтатусОтправления, ЦветаСтиля.ЦветФонаШапкиОтчета); 
	КонецЦикла;
...
Процедура ДатаДоставкиПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.СтатусОтправления) Тогда
		СписокОтправленийНаСервере("FBS");	
	КонецЕсли;	
КонецПроцедуры
...
&НаСервере
Процедура ЗапуститьОбработчикЗапросаИмпортаТоваровНаСервере(Services)
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");  
    ОбработкаОбъект.ОтправитьЗапросНаИмпортТоваров(Services); 
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
КонецПроцедуры  

&НаКлиенте
Процедура ЗапуститьОбработчикЗапросаИмпортаТоваров()  
	
	Если Не ЗначениеЗаполнено(Объект.ServicesImport) Тогда
		Возврат;	
	КонецЕсли; 
		
	ЗапуститьОбработчикЗапросаИмпортаТоваровНаСервере(Объект.ServicesImport);      
	
КонецПроцедуры

&НаСервере
Процедура ЗапуститьОбработчикИмпортаТоваровНаСервере(ДвоичныеДанные)
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");  
    ОбработкаОбъект.ИмпортироватьТовары(ДвоичныеДанные); 
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
КонецПроцедуры

&НаКлиенте
Процедура ЗапуститьОбработчикИмпортаТоваров(Команда) 
	
	ЕстьОшибки = Ложь;
	Если Не ЗначениеЗаполнено(Объект.ServicesImport) Тогда
		Сообщить("Не выбран сервис для импорта товара.");
		ЕстьОшибки = Истина;	
	КонецЕсли;

	Если Не ЗначениеЗаполнено(Объект.ОрганизацияИТ) Тогда
		Сообщить("Не заполнена организация для импорта товара.");
		ЕстьОшибки = Истина;	
	КонецЕсли;
	
	Если СтрНайти(Объект.ServicesImport, "SBER_MEGA_MARKET") = 0  Тогда 
		Если Не ЕстьОшибки Тогда
			ЗапуститьОбработчикЗапросаИмпортаТоваров(); 
		КонецЕсли;
		Возврат;
	КонецЕсли;	
	...
	Если Не ЕстьОшибки И ПроверитьСуществованиеФайла(ФайлИмпорта) = Ложь Тогда 
	...
	ЗапуститьОбработчикИмпортаТоваровНаСервере(ДвоичныеДанные);      
...
&НаСервере
Функция СписокСхемСопоставленияТовараНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	СписокСхемСопоставленияТовара = ОбработкаОбъект.ОбработатьСхемыСопоставленияТовара();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект"); 
	Возврат СписокСхемСопоставленияТовара;
КонецФункции 

&НаСервере
Функция СписокСтатусовОтправленийНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	СписокСтатусовОтправлений = ОбработкаОбъект.ОбработатьСписокОтправлений();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект"); 
	Возврат СписокСтатусовОтправлений;
КонецФункции
...
&НаСервере
Процедура УдалитьТоварыНаСайтеНаСервере()
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.УдалитьТоварыНаСайте();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");  
	
КонецПроцедуры

&НаКлиенте
Процедура УдалитьТоварыНаСайте(Команда)
	УдалитьТоварыНаСайтеНаСервере();
КонецПроцедуры 

&НаСервере
Процедура ОтвязатьСкладыНаСайтеНаСервере()

	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ОтвязатьСклады();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");  
	
КонецПроцедуры  

&НаКлиенте
Процедура ОтвязатьСкладыНаСайте(Команда) 
	...
	Иначе	   		
		ОтвязатьСкладыНаСайтеНаСервере();		
	КонецЕсли;
...
Процедура ПоискПоШтрихкодуВыполнить(Команда)
	...
	ОчиститьСообщения();
	//1//Оповещение = Новый ОписаниеОповещения("ПоискПоШтрихкодуЗавершение", ЭтотОбъект);
	//1//ШтрихкодированиеНоменклатурыБПКлиентСервер.ПоказатьВводШтрихкода(Оповещение); 
		
КонецПроцедуры  

&НаСервере
Процедура ОбработатьКомплектацииНаСервере()
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ОбработатьКомплектации();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
	
КонецПроцедуры  

&НаСервере
Процедура ОбработатьМаркировкуТовараНаСервере(ДанныеШтрихкода)
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ОбработатьМаркировкуТовара(ДанныеШтрихкода);;
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
	
КонецПроцедуры 

&НаСервере
Процедура ОбработатьЗаказыНаКомплектациюНаСервере(ДанныеШтрихкода)
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ОбработатьЗаказыНаКомплектацию(ДанныеШтрихкода);
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
	
КонецПроцедуры 

&НаСервере
Процедура ПеревестиКомплектациюВНовоеСостояниеНаСервере()
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ПеревестиКомплектациюВНовоеСостояниеНаСервере();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
...
Процедура ПоискПоШтрихкодуЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ОбработатьЗаказыНаКомплектациюНаСервере(ДанныеШтрихкода);
...
Процедура ВводШтрихкодаПриИзменении() Экспорт
	...
	ДанныеШтрихкода.Вставить("Штрихкод", СокрЛП(Объект.ВводШтрихкода));   
	ДанныеШтрихкода.Вставить("Количество", 1); 
	
	Если Объект.СостояниеКомплектации = 0 Тогда	
		
		//отрабатываем состояние стадии получения информации товаров на упаковку 	
		ОбработатьШтрихкоды(ДанныеШтрихкода);  
		ОбработатьЗаказыНаКомплектациюНаСервере(ДанныеШтрихкода); 
		ПеревестиКомплектациюВНовоеСостояниеНаСервере(); 
		Если Объект.СостояниеКомплектации = 0 Тогда  
			ОбработатьКомплектацииНаСервере(); 
	    КонецЕсли;		
		
	ИначеЕсли Объект.СостояниеКомплектации = 1 Тогда
		
		//отрабатываем состояние стадии маркировки товара 
		ОбработатьМаркировкуТовараНаСервере(ДанныеШтрихкода);
		ОбработатьКомплектацииНаСервере(); 
	...
	УстановитьОформлениеВводаШтрихкодаНаСервере();
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	//1//Если Не ШтрихкодированиеНоменклатурыБПКлиентСервер.ШтрихкодыВалидны(ДанныеШтрихкодов) Тогда
	//1//	Возврат;
	//1//КонецЕсли;
	...
	Если ПравоРегистрацииШтрихкодовНоменклатурыДоступно
		...
		//1//ШтрихкодированиеНоменклатурыБПКлиентСервер.ОбработатьНеизвестныеШтрихкоды(СтруктураПараметровДействия, КэшированныеЗначения, ЭтаФорма);
...
Функция ДобавитьНоменклатуруПоШтрихкодам(ДанныеШтрихкодов)
	...
	//1//Таблица.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	...
	//1//КэшированныеЗначения = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	КэшированныеЗначения = "";
	
	КэшированныеЗначения.ПравоРегистрацииШтрихкодовНоменклатурыДоступно = ПравоРегистрацииШтрихкодовНоменклатурыДоступно;
	
	//1//ПрефиксыВесовыхШтрихкодов = ЗначениеНастроекПовтИсп.ПолучитьПрефиксыВесовыхШтрихкодов();
...
Процедура ДобавитьТовары(Таблица)
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда 
			НоваяСтрока.oneCId = ПолучитьGIUDТовараНаСервере(НоваяСтрока.Номенклатура); 
...
Процедура СписокОтправленийНаСервере(ВидПродажи)
	...
	//тут больше ничего не делаем, перенос всей процедуры заполнения теперь по кнопке
	//выводим сообщение, чтобы клиент не запутался, что делать дальше после заполнения фильтров
	Сообщить("Для формирования окончательного списка заказов нажмите кнопку ""Заполнить"".", СтатусСообщения.Информация);

КонецПроцедуры 

&НаСервере
Процедура СписокОтправленийИтоговыйНаСервере(ВидПродажи)
...
&НаКлиенте
Процедура ЗаполнитьЗаказыПокупателей(Команда)
	СписокОтправленийИтоговыйНаСервере("FBS");
КонецПроцедуры
...
Процедура ПечатьЭтикетокЗаказов(Команда)
	...
	Если Объект.ФайлыДляСкачивания.Количество() > 0 Тогда 
		Элементы.ЗаказыФайлыДляСкачивания.Видимость = Истина;
		Элементы.ЗаказыФайлыДляСкачивания.ТекущаяСтрока  = Объект.ФайлыДляСкачивания[0].ПолучитьИдентификатор();  
	КонецЕсли;
КонецПроцедуры  
...
&НаСервере
Процедура ОтправитьЗаказыНаСборкуНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ОтправитьЗаказыНаСборку();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьЗаказыНаСборку(Команда)
	ОтправитьЗаказыНаСборкуНаСервере();
КонецПроцедуры  

&НаСервере
Процедура УкомплектоватьЗаказыПокупателейНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект"); 
	ОбработкаОбъект.ВыполнитьКоманду("УкомплектоватьЗаказыВSelSup"); 
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");  
КонецПроцедуры

&НаКлиенте
Процедура УкомплектоватьЗаказыПокупателей(Команда)
	УкомплектоватьЗаказыПокупателейНаСервере();
КонецПроцедуры

&НаСервере
Процедура ПометитьЗаказыСобраннымиНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ПометитьЗаказыСобранными();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
КонецПроцедуры

&НаКлиенте
Процедура ПометитьЗаказыСобранными(Команда)
	ПометитьЗаказыСобраннымиНаСервере();
КонецПроцедуры 

&НаСервере
Процедура ЗаполнитьНастройкиНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ЗаполнитьНастройкиПоУмолчанию(ОбработкаОбъект.Организация, Истина);
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
КонецПроцедуры
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Оповещение = Новый ОписаниеОповещения("ПродолжитьОперациюИзмененияОрганизации", ЭтотОбъект);
	Сообщение = НСтр("ru='Перезаполнить настройки?'");
	ПоказатьВопрос(Оповещение, Сообщение, РежимДиалогаВопрос.ДаНет); 

КонецПроцедуры  

&НаКлиенте
Процедура ПродолжитьОперациюИзмененияОрганизации(РезультатВопроса, ПараметрыЗаписи) Экспорт
...
&НаСервере
Процедура ВключитьТестовыйРежимПриИзмененииНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ИзменитьРежимПодключения();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
КонецПроцедуры

&НаКлиенте
Процедура ФайлыДляСкачиванияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ТекущиеФайлыДляСкачивания = Элементы.ЗаказыФайлыДляСкачивания.ТекущиеДанные; 
	
	Если Не ТекущиеФайлыДляСкачивания = Неопределено Тогда 
		ПерейтиПоНавигационнойСсылке(ТекущиеФайлыДляСкачивания.URL);	
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ЗакрытьФайлыДляСкачивания(Команда)
	Элементы.ЗаказыФайлыДляСкачивания.Видимость = Ложь;
КонецПроцедуры 

&НаКлиенте
Процедура КомплектацияФайлыДляСкачиванияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ТекущиеФайлыДляСкачивания = Элементы.КомплектацияФайлыДляСкачивания.ТекущиеДанные; 
...
&НаКлиенте
Процедура ЗакрытьКомплектацияФайлыДляСкачивания(Команда)
	Элементы.КомплектацияФайлыДляСкачивания.Видимость = Ложь;
КонецПроцедуры  

&НаСервере
Процедура КомплектацияОперацияСбросаНаСервере()
	Объект.Комплектация.Очистить();  
	Объект.СостояниеКомплектации = 0;  
	УстановитьОформлениеВводаШтрихкодаНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура КомплектацияОперацияСброса(Команда)
	КомплектацияОперацияСбросаНаСервере();
КонецПроцедуры
#КонецОбласти
...
&НаСервере
Функция ПолучитьКодТовараНаСервере(Номенклатура)
...
Процедура ВыбратьПоФильтру()	
	Для Каждого СтрокаТаблицы Из Объект.Заказы Цикл
		Если Элементы.Заказы.ПроверитьСтроку(СтрокаТаблицы.ПолучитьИдентификатор()) Тогда
			СтрокаТаблицы.Обработать = Истина;	
		Иначе    
...
Процедура СкладыВыбратьВсеФлажки()	
	Для Каждого СтрокаТаблицы Из Объект.Склады Цикл
		СтрокаТаблицы.ОбработатьСклады = Истина;
	КонецЦикла;
КонецПроцедуры 

&НаКлиенте
Процедура СкладыВыбратьПоФильтру()	
	Для Каждого СтрокаТаблицы Из Объект.Склады Цикл
		Если Элементы.Склады.ПроверитьСтроку(СтрокаТаблицы.ПолучитьИдентификатор()) Тогда
			СтрокаТаблицы.ОбработатьСклады = Истина;	
		Иначе    
			СтрокаТаблицы.ОбработатьСклады = Ложь;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура ТоварыВыбратьПоФильтру()	
	Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
		Если Элементы.Товары.ПроверитьСтроку(СтрокаТаблицы.ПолучитьИдентификатор()) Тогда
			СтрокаТаблицы.ОбновитьОстаток = Истина;	
		Иначе    
...
&НаКлиенте
Процедура ТоварыSelSupВыбратьПоФильтру()	
	Для Каждого СтрокаТаблицы Из Объект.ТоварыSelSup Цикл
		Если Элементы.ТоварыSelSup.ПроверитьСтроку(СтрокаТаблицы.ПолучитьИдентификатор()) Тогда
			СтрокаТаблицы.createlink = Истина;	
		Иначе    
...
Процедура УстановитьОформлениеНаСервере()  
	...
	Элементы.ВыбПрефикс.Видимость                        = Не Объект.ИспользоватьРасширеннуюНастройкуЗаказов;  
	Элементы.ВыбИспользоватьБонусныеЦены.Видимость       = Не Объект.ИспользоватьРасширеннуюНастройкуЗаказов;
	...
	Элементы.ИзменитьServicesImport_AVITO.Видимость            = Ложь;
	Элементы.ИзменитьServicesImport_LEROY_MERLIN.Видимость     = Ложь;   
    Элементы.ИзменитьServicesImport_DETMIR.Видимость           = Ложь; 
	Элементы.ИзменитьServicesImport_KAZAN_EXPRESS.Видимость    = Ложь;	
	Элементы.ИзменитьServicesImport_ALIE_EXPRESS.Видимость     = Ложь; 	
	...
	Если ЗначениеЗаполнено(Объект.ApiKey) Тогда
		...
		Если Не СписокМаркетплейсов = Неопределено Тогда 
			...
			Элементы.ИзменитьServicesImport_WILDBERRIES.Видимость      = НЕ (СписокМаркетплейсов.НайтиПоЗначению("WILDBERRIES") = Неопределено);    
			Элементы.ИзменитьServicesImport_OZON.Видимость             = НЕ (СписокМаркетплейсов.НайтиПоЗначению("OZON") = Неопределено); 
			Элементы.ИзменитьServicesImport_YANDEX_MARKET.Видимость    = НЕ (СписокМаркетплейсов.НайтиПоЗначению("YANDEX_MARKET") = Неопределено);  
			Элементы.ИзменитьServicesImport_SBER_MEGA_MARKET.Видимость = НЕ (СписокМаркетплейсов.НайтиПоЗначению("SBER_MEGA_MARKET") = Неопределено); 
			
			Элементы.ИзменитьServicesImport_AVITO.Видимость            = НЕ (СписокМаркетплейсов.НайтиПоЗначению("AVITO") = Неопределено);    
			Элементы.ИзменитьServicesImport_LEROY_MERLIN.Видимость     = НЕ (СписокМаркетплейсов.НайтиПоЗначению("LEROY_MERLIN") = Неопределено); 
			Элементы.ИзменитьServicesImport_DETMIR.Видимость           = НЕ (СписокМаркетплейсов.НайтиПоЗначению("DETMIR") = Неопределено);  
			Элементы.ИзменитьServicesImport_KAZAN_EXPRESS.Видимость    = НЕ (СписокМаркетплейсов.НайтиПоЗначению("KAZAN_EXPRESS") = Неопределено);  
			Элементы.ИзменитьServicesImport_ALIE_EXPRESS.Видимость     = НЕ (СписокМаркетплейсов.НайтиПоЗначению("ALIE_EXPRESS") = Неопределено); 
	...
	СписокСхемСопоставленияТовара = СписокСхемСопоставленияТовараНаСервере(); 
	СхемаСопоставленияТовараСписокВыбора = Элементы.СхемаСопоставленияТовара.СписокВыбора; 
	СхемаСопоставленияТовараСписокВыбора.Очистить();
	Для каждого ЭлементСписка Из СписокСхемСопоставленияТовара Цикл  
		СхемаСопоставленияТовараСписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление); 
	КонецЦикла;	
	
КонецПроцедуры  

&НаСервере
Процедура УстановитьОформлениеВводаШтрихкодаНаСервере()  
			
	Если Объект.СостояниеКомплектации = 0 Тогда 
		
		Элементы.ГруппаСборкиЗаказа.Заголовок           = "Отсканируйте штрих-код товара"; 
		Элементы.ВводШтрихкода.ПодсказкаВвода           = "Поиск по штрих-коду";
		Элементы.ГруппаСборкиЗаказа.ЦветТекстаЗаголовка = ЦветаСтиля.ЦветАкцента;  
		
	ИначеЕсли Объект.СостояниеКомплектации = 1 Тогда 
		
		Элементы.ГруппаСборкиЗаказа.Заголовок           = "Отсканируйте код маркировки"; 
		Элементы.ВводШтрихкода.ПодсказкаВвода           = "Прикреплене кода маркировки к заказу";  
		Элементы.ГруппаСборкиЗаказа.ЦветТекстаЗаголовка = ЦветаСтиля.ЦветОсобогоТекста;		
		
	КонецЕсли;  
	
	Если Объект.ФайлыДляСкачивания.Количество() > 0 Тогда 
		Элементы.КомплектацияФайлыДляСкачивания.Видимость = Истина;
		Элементы.КомплектацияФайлыДляСкачивания.ТекущаяСтрока  = Объект.ФайлыДляСкачивания[0].ПолучитьИдентификатор();  
	КонецЕсли;
	
КонецПроцедуры  

&НаСервере
Процедура УстановитьОформлениеИмпортаТовараНаСервере()  
			
	Элементы.ФайлИмпорта.Видимость = (СтрНайти(Объект.ServicesImport, "SBER_MEGA_MARKET") > 0);
...
Функция ПроверкаТиповРеквизитов()
	...
	ИначеЕсли  Объект.ИмяМетаданных = "УправлениеТорговлей" 
		ИЛИ Объект.ИмяМетаданных = "КомплекснаяАвтоматизация" Тогда
...
&НаКлиенте
Процедура СхемаСопоставленияТовараПриИзменении(Элемент) 
...
Процедура ОбновитьПродуктыSelSupНаСервере() 
	...
	ТаблицаНоменклатурыSelSup = ОбработкаОбъект.НайтиПродуктыSelSup(ОбработкаОбъект.TextQuery, ОбработкаОбъект.Services, ОбработкаОбъект.ЧислоТоваров); 
...
Процедура СопоставитьНоменклатурыСпискомНаСервере()
	...
	Для каждого ТекущиеДанныеТоварыSelSup Из Объект.ТоварыSelSup Цикл
		...
		Если Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод" 
			ИЛИ Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод_БезУчетаХарактеристик" Тогда
		...
		ИначеЕсли ТекущиеДанныеТовары1C.Количество() > 0 Тогда 
			Если Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод" Тогда
				OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ТекущиеДанныеТоварыSelSup.onecArticle, ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Характеристика));  			
			Иначе
				OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ТекущиеДанныеТоварыSelSup.onecArticle, ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C[0].Номенклатура));
			КонецЕсли;
...
Процедура УдалитьСопоставлениеНоменклатурыСпискомНаСервере()
	...
	Для каждого ТекущиеДанныеТоварыSelSup Из Объект.ТоварыSelSup Цикл
		...
		Если Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод"
			ИЛИ Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод_БезУчетаХарактеристик" Тогда
			Отбор.Вставить("barcode", ТекущиеДанныеТоварыSelSup.barcode); //добавить артикулы тоже - TODO ???   
		КонецЕсли; 	
		...
		Если ТекущиеДанныеТовары1C.Количество() = 0 Тогда   
			...
			OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ТекущиеДанныеТоварыSelSup.onecArticle, NULL);
			Если Не OneCId = Неопределено Тогда   
				ТекущиеДанныеТоварыSelSup.OneCId = OneCId;
			КонецЕсли;		
		ИначеЕсли ТекущиеДанныеТовары1C.Количество() > 1 Тогда    
...
&НаСервере
Процедура СопоставитьВсеНоменклатурыСпискомНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");  
    ОбработкаОбъект.СопоставитьТоварыНаСайте(); 
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");	
КонецПроцедуры

&НаКлиенте
Процедура СопоставитьВсеНоменклатурыСписком(Команда)
	СопоставитьВсеНоменклатурыСпискомНаСервере();
КонецПроцедуры
...
Процедура ПродолжитьОперациюСопоставленияТоваров(РезультатВопроса, ПараметрыЗаписи) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если Не ТекущиеДанныеТоварыSelSup = Неопределено 
			...
			Если Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод" Тогда
				OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ТекущиеДанныеТоварыSelSup.onecArticle, ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C.Характеристика));   //TODO
			Иначе
				OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.name, ТекущиеДанныеТоварыSelSup.id, ТекущиеДанныеТоварыSelSup.onecArticle, ПолучитьGIUDТовараНаСервере(ТекущиеДанныеТовары1C.Номенклатура));  
			КонецЕсли;
			Если Не OneCId = Неопределено Тогда   
				ТекущиеДанныеТоварыSelSup.OneCId = OneCId;
			КонецЕсли;
...
Процедура ПродолжитьОперациюУдаленияСопоставленияТоваров(РезультатВопроса, ПараметрыЗаписи) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если Не ТекущиеДанныеТоварыSelSup = Неопределено Тогда
			OneCId = СопоставитьНоменклатурыНаСервере(ТекущиеДанныеТоварыSelSup.Name, ТекущиеДанныеТоварыSelSup.id, NULL, NULL);
			Если Не OneCId = Неопределено Тогда   
				ТекущиеДанныеТоварыSelSup.OneCId = OneCId;
			КонецЕсли;	
		КонецЕсли; 
...
Процедура СписокТоваровSelSupПриАктивизацииСтроки(Элемент)
	...
	Иначе
		...
		ТекущийBarcode = ТекущиеДанные.barcode;  
		ТекущийArticle = ТекущиеДанные.article; 
		//ТекущийOzonArticle = ТекущиеДанные.ozonArticle;
		//ТекущийSberArticle = ТекущиеДанные.sberArticle;
		//ТекущийYandexMarketShopSku = ТекущиеДанные.yandexMarketShopSku;
		...
		Если (Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод" ИЛИ Объект.СхемаСопоставленияТовара = "Штрихкод_Штрихкод_БезУчетаХарактеристик")
			И ЗначениеЗаполнено(ТекущийBarcode) Тогда
			Элементы.Товары1C.ОтборСтрок = Новый ФиксированнаяСтруктура("id, barcode", ТекущийId, ТекущийBarcode); 
		//ИначеЕсли ЗначениеЗаполнено(ТекущийArticle) Тогда
		//	Элементы.Товары1C.ОтборСтрок = Новый ФиксированнаяСтруктура("id, article", ТекущийId, ТекущийArticle);	//TODO		
		Иначе
...
Процедура Товары1CПриИзменении(Элемент)
	...
	Если Не ТекущиеДанныеТоварыSelSup = Неопределено Тогда 
		...
		ТекущиеДанныеТовары1C.barcode = ТекущиеДанныеТоварыSelSup.barcode; 
		ТекущиеДанныеТовары1C.Код     = ПолучитьКодТовараНаСервере(ТекущиеДанныеТовары1C.Номенклатура);   //TODO
...
&НаКлиенте
Процедура ТоварыSelSupВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) 
	
	ТекущиеДанныеТоварыSelSup = Элементы.ТоварыSelSup.ТекущиеДанные; 
	
	Если Не ТекущиеДанныеТоварыSelSup = Неопределено Тогда 
		ПерейтиПоНавигационнойСсылке(ТекущиеДанныеТоварыSelSup.linksNoBlock);	
	КонецЕсли;
...
Процедура СохранитьНастройкиНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ВыгрузитьНастройки();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект")
КонецПроцедуры
